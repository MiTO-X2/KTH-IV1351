...
CREATE TABLE rental (
    ...
    terminated INT NOT NULL CHECK (terminated IN (0, 1)),
    student_id INT NOT NULL,
    ...
    --CONSTRAINT rental_end_date__limit2 CHECK (rental_end_date >= rental_start_date)
);
...
RETURNS trigger AS $$
BEGIN
    -- Skip check for terminations (as we are about to terminate a rental)
    IF NEW.terminated = 0 THEN
        -- Check if the student already has 2 active rentals
        IF (SELECT COUNT(*) 
            FROM rental 
            WHERE student_id = NEW.student_id 
              AND terminated = 0) >= 2 THEN
...
-- Trigger function for ensuring rental_end_date >= rental_start_date
CREATE OR REPLACE FUNCTION check_rental_end_date_validity() 
RETURNS TRIGGER AS $$
BEGIN
    -- Check if rental_end_date is before rental_start_date
    IF NEW.rental_end_date < NEW.rental_start_date THEN
        RAISE EXCEPTION 'End date must be greater than or equal to start date. Rental Start: %, Rental End: %',
...
-- Trigger to check rental_end_date >= rental_start_date on INSERT and UPDATE
CREATE TRIGGER rental_end_date_check
BEFORE INSERT OR UPDATE ON rental
FOR EACH ROW
EXECUTE FUNCTION check_rental_end_date_validity();

-- Trigger function for ensuring rental_end_date is within 12 months of rental_start_date
CREATE OR REPLACE FUNCTION check_rental_end_date_limit() 
RETURNS TRIGGER AS $$
BEGIN
    -- Check if rental_end_date is within 12 months from rental_start_date
    IF NEW.rental_end_date > NEW.rental_start_date + INTERVAL '12 months' THEN
        RAISE EXCEPTION 'End date must be within 12 months from start date. Rental Start: %, Rental End: %',
            NEW.rental_start_date, NEW.rental_end_date;
    END IF;
    RETURN NEW;
END;
...
-- Trigger to check rental_end_date is within 12 months of rental_start_date on INSERT and UPDATE
CREATE TRIGGER rental_end_date_limit_check
BEFORE INSERT OR UPDATE ON rental
FOR EACH ROW
EXECUTE FUNCTION check_rental_end_date_limit();
